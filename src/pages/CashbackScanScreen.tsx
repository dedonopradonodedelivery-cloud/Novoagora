...
export const CashbackScanScreen: React.FC<CashbackScanScreenProps> = ({
  ...
  // Referência para a função de scan
  const scanFrame = useRef<() => void>(() => {});
  ...
  const handleScan = (qrData: string) => {
    ...
    try {
      ...
      if (
        data.type === "localizei_cashback_qr" &&
        data.merchantId &&
        data.storeId
      ) {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Formato de QR Code incorreto."
      );
      // Reinicia a leitura depois de alguns segundos (apenas se tiver detector)
      setTimeout(() => {
        setError(null);
        if (hasDetector && scanFrame.current) {
          scanFrame.current();
        }
      }, 3000);
    }
  ...
  useEffect(() => {
    ...
    // 1) Verifica suporte ao BarcodeDetector, mas NÃO bloqueia a câmera
    if ("BarcodeDetector" in window) {
      ...
      // Sem detector → ainda assim mostra câmera, mas sem leitura automática
      setHasDetector(false);
      setError(
        "Seu navegador não suporta leitura automática de QR Code. A câmera será exibida, mas pode ser necessário atualizar o sistema."
      );
    }

    // 2) Liga a câmera SEMPRE
    const setupCamera = async () => {
      try {
        ...
        if (videoRef.current) {
          ...
          // Só inicia leitura se tiver detector
          if (hasDetector && scanFrame.current) {
            animationFrameId.current = requestAnimationFrame(scanFrame.current);
          }
        }
        ...
        if (isIOS) {
          setError(
            "No iPhone, abra no Safari com HTTPS e permita o acesso à câmera nas configurações do site."
          );
        } else {
          setError(
  ...
  }, [isIOS, hasDetector]); // deps estáveis
      ...
      <div className="relative z-20 flex flex-col h-full">
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          <div className="relative w-64 h-64">
            ...
            {isScanning && hasDetector && (
              <div className="absolute inset-x-2 top-2 h-0.5 bg-red-500 shadow-[0_0_15px_2px_rgba(239,68,68,0.6)] animate-[scan_3s_infinite_ease-in-out]" />
            )}
          </div>
          ...
          {!hasDetector && (
            <p className="text-[11px] text-white/70 mt-3 max-w-xs text-center">
              Seu navegador pode não suportar leitura automática de QR Code.
              Recomendamos manter o iOS e o navegador atualizados.
            </p>
          )}
        </div>
        ...
        <div className="p-5 pb-8 min-h-[80px]">
          ...
          {!isScanning && !error && (
            <div className="flex flex-col items-center text-center text-white">
              ...
              <p className="font-bold">Câmera inicializando...</p>
            </div>
          )}
        </div>
      </div>
